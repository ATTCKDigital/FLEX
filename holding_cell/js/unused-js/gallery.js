...
function Gallery ($el) {
	function nextSlide(e) {
		...
		// var nextDot = $el.find('.gallery-nav-item[data-slide-id="slide-'+nextSlide+'"]')
		...
		if ($(next).length) {
			...
			//update the next dot
			// $(nextDot).addClass('activeTab').siblings().removeClass('activeTab'); 

		} else {
			//if the next slide is the last slide, go back to the first slide
			$el.find('.gallery-popup-item:first-child').addClass('activeSlide').siblings().removeClass('activeSlide');
			// $el.find('.gallery-nav-item[data-slide-id="slide-1"]').addClass('activeTab').siblings().removeClass('activeTab');
		}
	}
	...
	function prevSlide() {
		...
		// var prevDot = $el.find('.gallery-nav-item[data-slide-id="slide-'+prevSlide+'"]')
		...
		if ($(prev).length) {
			...
			//update the next dot
			// $(prevDot).addClass('activeTab').siblings().removeClass('activeTab'); 


		} else {
			console.log($el.find('.gallery-item:last-child'))
			//if the prev slide is the last slide, go back to the first slide
			$el.find('.gallery-popup-item:last-child').addClass('activeSlide').siblings().removeClass('activeSlide');
			// $el.find('.gallery-nav-item:last-child').addClass('activeTab').siblings().removeClass('activeTab');
	...
	this.init = function($el) {
		$el.find('#closeGallery').on('click', closeSlideshow);
		$el.find('.gallery-item').on('click', openSlideshow);
