...
FLEX.GlobalEvents.initGlobalEvents = function () {
	...
	function cancelRelativePathLinkClicks() {
		...
		$( 'body' ).on( 'click', 'a', function ( e ) {
			...
			if ( hasHrefVal > -1 ) {
				...
				console.log(
					'/FLEX/\tjs/\tglobal-events.js',
					'initGlobalEvents().cancelRelativePathLinkClicks() > link click canceled, $(this): e: ',
					$( this ),
					e
				);
			}
	...
	$window.on( 'scroll', function ( e ) {
		...
		$( document.body ).trigger( 'FLEX.scroll', {
			...
			scrollDirection:
				G.currentScrollTop > G.lastScrollTop ? 'down' : 'up',
		} );
	...
	function detectOrientation() {
		...
		$( document.body ).removeClass(
			'orientation-portrait orientation-landscape video-portrait video-landscape short-screen tall-screen'
		);
	...
	function detectViewportHeightRatio() {
		...
		if ( totalComponentHeight > G.viewportHeight ) {
			// Set new detection
			$( document.body ).addClass(
				'body-content-height-is-taller-than-viewport'
			);
		} else {
	...
	$( window ).on( 'load', function () {
		...
		setTimeout( function () {
			$( document.body ).addClass(
				'window-has-been-loaded-for-two-seconds'
			);
		}, 2000 );
	...
	$( function () {
		...
		setTimeout( function () {
			$( document.body ).addClass(
				'dom-has-been-loaded-for-five-seconds'
			);
		}, 5000 );
	} );
