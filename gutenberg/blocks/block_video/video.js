...
// Register block
export default registerBlockType( 'flexlayout/video', {
	title: __( 'Video' ),
	...
	edit: ( props ) => {
		...
		const setUploadVideo = ( value ) =>
			props.setAttributes( { uploadVideo: value } );
		const removeUploadVideo = () =>
			props.setAttributes( { uploadVideo: null } );

		const setYoutubeVideo = ( value ) =>
			props.setAttributes( { youtubeVideo: value } );
		...
		const setBrightcoveAccount = ( value ) =>
			props.setAttributes( { brightcoveAccount: value } );
		const setBrightcoveVideo = ( value ) =>
			props.setAttributes( { brightcoveVideo: value } );

		const setVideoThumbnail = ( value ) =>
			props.setAttributes( { videoThumbnail: value } );
		const removeVideoThumbnail = () =>
			props.setAttributes( { videoThumbnail: null } );
				...
				<div className="video-container">
					...
					{ props.isSelected ? (
						<div className="media-button-wrapper">
							...
							<p>
								{ __(
									'Add/Upload a 1920x1080 video thumbnail.'
								) }
							</p>
						</div>
					) : null }
			...
			if ( ! props.attributes.uploadVideo ) {
				return (
					<div className="media-upload-wrapper">
						...
						<p>{ __( 'Add/Upload an .mp4 video file.' ) }</p>
					</div>
				);
			}
					...
					<p>
						...
						>
							<source
								type="video/mp4"
								src={
									props.attributes.uploadVideo
										? props.attributes.uploadVideo.url
										: ''
								}
							/>
						</video>
		...
		const youtubeVideoSelect = () => {
			...
			return (
				<TextControl
					...
					help={ __(
						'Paste the ID of the YouTube Video.',
						'flexlayout'
					) }
					value={
			...
			return (
				<div>
					<TextControl
						...
						help={ __(
							'Paste the ID of the BrightCove Account.',
							'flexlayout'
						) }
						value={
					...
					<TextControl
						...
						help={ __(
							'Paste the ID of the BrightCove Video.',
							'flexlayout'
						) }
						value={
			...
			if (
				...
				return (
					<div className="flex-center-center">
						...
						<p>
							{
								'Choose a video type from the block components panel.'
							}
						</p>
					</div>
				);
