...
function Nav( $el ) {
	...
	function bindEvents() {
		...
		if ( $( window ).width() > 1024 ) {
			$el.find( '.menu-items > .menu-item-has-children' ).on(
				'mouseenter',
				openSubNav
			);
			...
			$( document ).on( 'mouseover', function ( e ) {
				...
				if (
					...
					! $target.closest( '.main-header' ).length
				) {
					closeSubNav( e );
				}

				if (
					! $target.is( '.menu-item-has-children' ) &&
			...
			$el.find( '.menu-item-has-children > a' ).on(
				'click',
				toggleSubNav
			);
		}
